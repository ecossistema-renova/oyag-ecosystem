...
async function overview(){
 ...
 '<section class="module-launchpad"><a href="./marketplace.html"><span>◈</span><div><strong>Marketplace</strong><small>Produtos, serviços e compras</small></div></a><a href="https://academy.cledemilsonoliveira.com/"><span>⌘</span><div><strong>OYAG Academy</strong><small>Aprendizado e progresso</small></div></a><a href="./community-app.html"><span>✦</span><div><strong>OYAG Community</strong><small>Troca de conhecimento</small></div></a><a href="./minhas-financas.html"><span>◉</span><div><strong>OYAG Finance</strong><small>Organização financeira</small></div></a></section>'+
...
async function showAffiliateNetwork(){
 C.innerHTML='<div class="loading">Carregando sua Central de Afiliados…</div>';
 ...
 const [{data:dash,error:dashError},{data:pipeline,error:pipelineError},{data:materials,error:materialsError}]=await Promise.all([
  sb.rpc('oyag_my_affiliate_dashboard'),
  sb.rpc('oyag_affiliate_sales_pipeline',{p_limit:150}),
  sb.from('oyag_affiliate_marketing_materials').select('id,product_code,product_name,title,format,asset_url,headline,caption,cta_text,theme,sort_order,status').eq('status','active').order('product_name').order('sort_order')
 ]);
 if(dashError){C.innerHTML=statePanel('Não foi possível carregar seu painel',dashError.message);return}
 const d=dash||joined||{},t=d.totals||{},media=materialsError?[]:(materials||[]);
 const pipelineData=pipelineError?{}:(pipeline||{}),pipelineRows=Array.isArray(pipelineData.rows)?pipelineData.rows:[],pipelineAdmin=Boolean(pipelineData.is_admin);
 const affiliateUrl=d.share_url||joined?.share_url||'';
 const savedPixKey=d.pix_key||'',maskedPixKey=savedPixKey?('••••••••'+savedPixKey.slice(-4)):'';
 ...
 const formatLabel={story:'Story 9:16',reel_cover:'Capa de Reels',feed_square:'Feed quadrado',feed_portrait:'Feed vertical',whatsapp_status:'Status WhatsApp'};
 const products=[...new Map(media.map(x=>[x.product_code,x.product_name])).entries()];
 const materialCards=media.map(m=>{
  const theme=m.theme||{},from=theme.from||'#07152d',to=theme.to||'#244eb9',accent=theme.accent||'#46c8ff';
  return '<article class="creative-card" data-material-card data-product="'+esc(m.product_code)+'" data-format="'+esc(m.format)+'"><div class="creative-preview" style="--creative-from:'+esc(from)+';--creative-to:'+esc(to)+';--creative-accent:'+esc(accent)+'">'+(m.asset_url?'<img src="'+esc(m.asset_url)+'" alt="">':'<img src="./assets/oyag-approved-mark.png" alt="OYAG">')+'<span>'+esc(m.product_name)+'</span><strong>'+esc(m.headline)+'</strong><small>'+esc(m.cta_text)+'</small></div><div class="creative-info"><div><span class="creative-format">'+esc(formatLabel[m.format]||m.format)+'</span><h3>'+esc(m.title)+'</h3></div><p>'+esc(m.caption)+'</p><div class="creative-actions"><button class="button secondary" data-copy-caption="'+esc(m.id)+'">Copiar legenda</button><button class="catalog-primary" data-personalize-material="'+esc(m.id)+'">Criar com meu link</button></div></div></article>';
 }).join('');
 const paymentLabel={paid:'Pago',confirmed:'Pago',approved:'Pago',pending:'Aguardando',pending_payment:'Aguardando',failed:'Falhou',cancelled:'Cancelado',refunded:'Estornado'};
 const commissionLabel={pending:'Pendente',released:'Liberada',paid:'Paga',cancelled:'Cancelada'};
 const pipelineTable=pipelineRows.length?'<div class="table-wrap pipeline-table"><table><thead><tr><th>Entrada</th><th>Afiliado e origem</th><th>Lead</th><th>Produto ou interesse</th><th>Pagamento do cliente</th><th>Comissão</th><th>Prazo e alerta</th><th>Ação</th></tr></thead><tbody>'+pipelineRows.map(x=>{
  const payState=String(x.payment_status||x.order_status||'pending').toLowerCase(),commissionState=String(x.commission_state||'pending').toLowerCase();
  const paymentText=paymentLabel[payState]||x.payment_status||x.order_status||'Aguardando';
  const commissionText=x.commission_cents==null?'Ainda não gerada':formatMoney(x.commission_cents)+' · '+(commissionLabel[commissionState]||commissionState);
  const deadline=x.commission_paid_at?'Paga em '+formatDate(x.commission_paid_at):x.payment_due_at?((x.is_overdue?'Atrasada · ':'Até ')+formatDate(x.payment_due_at)):'Aguardando liberação';
  const action=pipelineAdmin&&x.entitlement_id&&commissionState==='released'?'<button class="catalog-primary pipeline-pay-action" data-mark-commission-paid="'+esc(x.entitlement_id)+'">Marcar como paga</button>':'<span class="pipeline-no-action">—</span>';
  return '<tr><td><strong>'+esc(x.order_number||'Entrada atribuída')+'</strong><small>'+formatDate(x.entered_at)+'</small></td><td><strong>'+esc(x.affiliate_code||'—')+'</strong><small>'+esc(x.lead_origin||'Link de afiliado')+'</small></td><td>'+esc(x.lead_name||'Lead identificado no checkout')+'</td><td>'+esc(x.product_interest||'Interesse ainda não informado')+'</td><td><span class="pipeline-status '+(paymentText==='Pago'?'paid':payState==='failed'||payState==='cancelled'?'overdue':'pending')+'">'+esc(paymentText)+'</span><small>'+formatMoney(x.total_cents||0)+'</small></td><td><span class="pipeline-status '+esc(commissionState)+'">'+esc(commissionText)+'</span></td><td><span class="pipeline-deadline '+(x.is_overdue?'overdue':'')+'">'+esc(deadline)+'</span></td><td>'+action+'</td></tr>';
 }).join('')+'</tbody></table></div>':statePanel('Nenhuma entrada atribuída ainda','Quando alguém entrar pelo link do afiliado, o acompanhamento aparecerá aqui.');

 C.innerHTML=
 '<div class="affiliate-hero premium"><div><p class="eyebrow">CENTRAL DE CRESCIMENTO</p><h2>Afiliados & Recrutamento</h2><p>Seu link, seus materiais e seus resultados organizados em um só lugar.</p></div><span class="affiliate-status '+esc(d.status||'pending')+'">'+esc(statusLabel)+'</span></div>'+
 cards([['Pendente',formatMoney(t.pending_cents||0),'aguardando liberação'],['Liberado',formatMoney(t.released_cents||0),'autorizado para pagamento'],['Pago',formatMoney(t.paid_cents||0),'histórico confirmado'],['Recrutados',Number(t.recruits||0),'um nível'],['Conversões',Number(t.conversions||0),'aquisições atribuídas']])+
 '<div class="affiliate-link-panel"><div class="affiliate-link-copy"><span>SEU LINK DE AFILIADO</span><strong>'+esc(affiliateUrl)+'</strong><small>Prioridade de atribuição válida por 30 dias.</small></div><div class="affiliate-link-actions"><button class="catalog-primary" id="affiliateCopy">⧉ Copiar link</button><span class="copy-feedback" id="affiliateCopyFeedback" role="status" aria-live="polite">✓ Link copiado</span><button class="button secondary" id="affiliateShare">↗ Compartilhar</button></div></div>'+
 '<section class="creative-hub"><div class="creative-hub-head"><div><p class="eyebrow">MATERIAIS DE DIVULGAÇÃO</p><h2>Escolha um produto e comece a divulgar</h2><p>Baixe artes personalizadas com o seu link e QR Code ou copie a legenda pronta.</p></div></div>'+
 '<div class="creative-filters"><select id="creativeProductFilter"><option value="">Todos os produtos</option>'+products.map(x=>'<option value="'+esc(x[0])+'">'+esc(x[1])+'</option>').join('')+'</select><select id="creativeFormatFilter"><option value="">Todos os formatos</option><option value="story">Story 9:16</option><option value="whatsapp_status">Status WhatsApp</option><option value="reel_cover">Capa de Reels</option><option value="feed_square">Feed quadrado</option><option value="feed_portrait">Feed vertical</option></select></div>'+
 '<div class="creative-grid" id="creativeGrid">'+(materialCards||statePanel('Materiais em preparação','As artes oficiais aparecerão aqui.'))+'</div></section>'+
 '<div class="affiliate-layout"><div class="panel payout-panel"><div class="panel-heading"><div><p class="eyebrow">PAGAMENTO</p><h2>Dados para recebimento</h2></div><span class="affiliate-status '+(d.pix_configured&&d.identity_configured?'active':'pending')+'">'+(d.pix_configured&&d.identity_configured?'Completo':'Pendente')+'</span></div>'+
 (d.pix_configured?'<div class="payout-summary"><div><span>Banco</span><strong>'+esc(d.pix_bank_name||'Não informado')+'</strong></div><div><span>Tipo da chave</span><strong>'+esc(({cpf:'CPF',cnpj:'CNPJ',email:'E-mail',phone:'Telefone',random:'Chave aleatória'})[d.pix_type]||d.pix_type||'—')+'</strong></div><div><span>Chave PIX</span><strong id="savedPixValue">'+esc(maskedPixKey)+'</strong><button class="payout-reveal" id="revealPixKey" type="button">Mostrar</button></div><div><span>CPF do titular</span><strong>***.***.***-'+esc(d.cpf_last4||'••••')+'</strong></div></div>':'<p class="muted payout-intro">Cadastre os dados do titular para receber comissões com segurança e evitar duplicidade de cadastro.</p>')+
 '<div class="payout-edit-note">✎ Você pode alterar o banco, o tipo e a chave PIX sempre que precisar.</div><form id="affiliatePixForm" class="affiliate-form payout-form"><label class="affiliate-field"><span>CPF do titular</span><input id="affiliateCpf" inputmode="numeric" autocomplete="off" maxlength="14" '+(d.identity_configured?'':'required')+' placeholder="'+(d.identity_configured?'CPF já validado — deixe em branco':'000.000.000-00')+'"><small>'+(d.identity_configured?'Preencha somente se precisar alterar o CPF.':'Usado para validar uma única conta de afiliado.')+'</small></label><label class="affiliate-field"><span>Banco ou instituição</span><input id="affiliateBank" maxlength="120" required placeholder="Ex.: Nubank, Itaú, Mercado Pago" value="'+esc(d.pix_bank_name||'')+'"></label><label class="affiliate-field"><span>Tipo da chave</span><select id="affiliatePixType" required><option value="cpf">CPF</option><option value="cnpj">CNPJ</option><option value="email">E-mail</option><option value="phone">Telefone</option><option value="random">Chave aleatória</option></select></label><label class="affiliate-field"><span>Chave PIX</span><input id="affiliatePixKey" maxlength="180" required placeholder="Informe sua chave PIX" value="'+esc(savedPixKey)+'"></label><button class="catalog-primary" type="submit">'+(d.pix_configured?'Salvar alterações':'Salvar dados de recebimento')+'</button></form></div>'+
 '<div class="panel"><div class="panel-heading"><div><p class="eyebrow">COMO FUNCIONA</p><h2>Divulgação rastreável</h2></div></div><ol class="affiliate-steps"><li><b>1</b><span>Escolha o produto e o formato.</span></li><li><b>2</b><span>Baixe a arte com seu QR Code.</span></li><li><b>3</b><span>Publique junto com sua legenda.</span></li><li><b>4</b><span>Acompanhe conversões e comissões.</span></li></ol></div></div>'+
 '<aside class="asaas-referral-card"><div class="asaas-referral-icon">A$</div><div><span>RECOMENDAÇÃO OYAG</span><h3>Organize suas movimentações com o Asaas</h3><p>Ainda não tem uma conta? A abertura é opcional e pode ajudar a centralizar cobranças e movimentações financeiras em um provedor utilizado pelo OYAG.</p></div><a class="asaas-referral-action" href="https://www.asaas.com/r/823b5bfe-e5e3-4d5f-85a0-ee2975dd102d" target="_blank" rel="noopener noreferrer sponsored">Abrir conta no Asaas ↗</a></aside>'+
 '<div class="panel pipeline-panel"><div class="panel-heading"><div><p class="eyebrow">FLUXO COMERCIAL</p><h2>Vendas e comissões</h2><p>Da entrada pelo link ao pagamento da comissão, com prazo operacional de até 7 dias após a liberação.</p></div><span class="pipeline-scope">'+(pipelineAdmin?'Visão administrativa':'Minhas indicações')+'</span></div>'+pipelineTable+'</div>'+
 (['owner','platform_admin'].includes(role)?'<div class="panel owner-material-admin"><div class="panel-heading"><div><p class="eyebrow">CONTA DONO</p><h2>Publicar material para toda a rede</h2></div></div><form id="newMaterialForm" class="owner-material-form"><input id="matProductCode" required placeholder="Código do produto (ex.: oyag-food)"><input id="matProductName" required placeholder="Nome do produto"><input id="matTitle" required placeholder="Nome interno do material"><select id="matFormat"><option value="story">Story 9:16</option><option value="whatsapp_status">Status WhatsApp</option><option value="reel_cover">Capa de Reels</option><option value="feed_square">Feed quadrado</option><option value="feed_portrait">Feed vertical</option></select><input id="matHeadline" required placeholder="Headline da arte"><input id="matAssetUrl" type="url" placeholder="URL da imagem (opcional)"><textarea id="matCaption" required placeholder="Legenda pronta para o afiliado"></textarea><button class="catalog-primary" type="submit">Publicar para a rede</button></form></div>'+
 '<div class="panel"><div class="panel-heading"><div><p class="eyebrow">APROVAÇÕES</p><h2>Afiliados pendentes</h2></div><span>'+approvals.length+' pendente(s)</span></div>'+(approvals.length?'<div class="table-wrap"><table><thead><tr><th>Código</th><th>Cadastro</th><th>PIX</th><th>Ação</th></tr></thead><tbody>'+approvals.map(x=>'<tr><td><strong>'+esc(x.affiliate_code)+'</strong><br><small>'+esc(x.user_id)+'</small></td><td>'+formatDate(x.created_at)+'</td><td>'+esc(x.pix_type||'Não informado')+'</td><td><button class="catalog-primary" data-approve-affiliate="'+esc(x.user_id)+'">Aprovar</button></td></tr>').join('')+'</tbody></table></div>':statePanel('Nenhuma aprovação pendente','Novas solicitações aparecerão aqui.'))+'</div>':'');

 const getMaterial=id=>media.find(x=>x.id===id);
 const filterMaterials=()=>{const p=C.querySelector('#creativeProductFilter').value,f=C.querySelector('#creativeFormatFilter').value;C.querySelectorAll('[data-material-card]').forEach(card=>card.hidden=Boolean((p&&card.dataset.product!==p)||(f&&card.dataset.format!==f)))};
 C.querySelector('#creativeProductFilter')?.addEventListener('change',filterMaterials);
 C.querySelector('#creativeFormatFilter')?.addEventListener('change',filterMaterials);
 C.querySelector('#affiliateCopy')?.addEventListener('click',async e=>{try{await navigator.clipboard.writeText(affiliateUrl);const b=e.currentTarget,f=C.querySelector('#affiliateCopyFeedback');b.classList.add('is-copied');b.textContent='✓ Copiado';f?.classList.add('show');clearTimeout(b._copyTimer);b._copyTimer=setTimeout(()=>{b.classList.remove('is-copied');b.textContent='⧉ Copiar link';f?.classList.remove('show')},2600)}catch{}});
 C.querySelector('#affiliateShare')?.addEventListener('click',async()=>{if(navigator.share)try{await navigator.share({title:'OYAG Ecosystem',text:'Conheça as soluções OYAG:',url:affiliateUrl})}catch{}else try{await navigator.clipboard.writeText(affiliateUrl);alert('Link copiado para compartilhar.')}catch{}});
 C.querySelectorAll('[data-copy-caption]').forEach(b=>b.addEventListener('click',async()=>{const m=getMaterial(b.dataset.copyCaption);const text=(m?.caption||'')+'\n\n'+affiliateUrl;try{await navigator.clipboard.writeText(text);alert('Legenda e link copiados.')}catch{}}));
 C.querySelectorAll('[data-personalize-material]').forEach(b=>b.addEventListener('click',()=>generateAffiliateStatus(getMaterial(b.dataset.personalizeMaterial),affiliateUrl)));
 if(d.pix_type)C.querySelector('#affiliatePixType').value=d.pix_type;
 C.querySelectorAll('#affiliatePixForm input,#affiliatePixForm select').forEach(field=>{field.disabled=false;field.readOnly=false});
 C.querySelector('#revealPixKey')?.addEventListener('click',e=>{const value=C.querySelector('#savedPixValue'),show=e.currentTarget.textContent==='Mostrar';value.textContent=show?savedPixKey:maskedPixKey;e.currentTarget.textContent=show?'Ocultar':'Mostrar'});
 C.querySelector('#affiliatePixForm')?.addEventListener('submit',async e=>{e.preventDefault();const b=e.submitter,original=b.textContent;b.disabled=true;b.textContent='Salvando…';const {error}=await sb.rpc('oyag_update_my_affiliate_payout',{p_pix_type:C.querySelector('#affiliatePixType').value,p_pix_key:C.querySelector('#affiliatePixKey').value.trim(),p_bank_name:C.querySelector('#affiliateBank').value.trim(),p_cpf:C.querySelector('#affiliateCpf').value.trim()});if(error){const messages={cpf_required:'Informe o CPF do titular para concluir o primeiro cadastro.',invalid_cpf:'Informe um CPF válido.',cpf_already_registered:'Este CPF já está vinculado a outra conta de afiliado.',invalid_bank_name:'Informe o banco ou a instituição de pagamento.',invalid_pix_key:'Informe uma chave PIX válida.'};alert(messages[error.message]||'Não foi possível salvar os dados agora.');b.disabled=false;b.textContent=original;return}alert('Dados de recebimento atualizados com segurança.');showAffiliateNetwork()});
 C.querySelectorAll('[data-mark-commission-paid]').forEach(b=>b.addEventListener('click',async()=>{if(!confirm('Confirmar que esta comissão já foi paga? Esta ação ficará registrada no histórico.'))return;const original=b.textContent;b.disabled=true;b.textContent='Registrando…';const {error}=await sb.rpc('oyag_mark_affiliate_commission_paid',{p_entitlement_id:b.dataset.markCommissionPaid});if(error){alert(error.message||'Não foi possível registrar o pagamento.');b.disabled=false;b.textContent=original;return}showAffiliateNetwork()}));
 C.querySelector('#newMaterialForm')?.addEventListener('submit',async e=>{e.preventDefault();const b=e.submitter;b.disabled=true;b.textContent='Publicando…';const row={product_code:C.querySelector('#matProductCode').value.trim().toLowerCase().replace(/[^a-z0-9]+/g,'-'),product_name:C.querySelector('#matProductName').value.trim(),title:C.querySelector('#matTitle').value.trim(),format:C.querySelector('#matFormat').value,headline:C.querySelector('#matHeadline').value.trim(),asset_url:C.querySelector('#matAssetUrl').value.trim()||null,caption:C.querySelector('#matCaption').value.trim(),cta_text:'Saiba mais',status:'active',created_by:session.user.id};const {error}=await sb.from('oyag_affiliate_marketing_materials').insert(row);if(error){alert(error.message);b.disabled=false;b.textContent='Publicar para a rede';return}alert('Material publicado para toda a rede.');showAffiliateNetwork()});
...
async function generateAffiliateStatus(material,affiliateUrl){
 if(!material||!affiliateUrl){alert('O link de afiliado ainda não está disponível.');return}
 if(typeof QRCode==='undefined'){alert('O gerador de QR Code ainda está carregando. Tente novamente em alguns segundos.');return}
 const modal=document.createElement('div');modal.className='creative-modal';modal.innerHTML='<div class="creative-modal-backdrop"></div><section class="creative-modal-card"><div><p class="eyebrow">ARTE PERSONALIZADA</p><h2>'+esc(material.product_name)+'</h2><p>Sua arte já inclui o seu link e QR Code exclusivo.</p></div><canvas id="affiliateCreativeCanvas" width="1080" height="1920"></canvas><div class="creative-modal-actions"><button class="button secondary" data-close-creative>Fechar</button><button class="catalog-primary" id="downloadCreative">Baixar Status 1080×1920</button></div></section>';document.body.appendChild(modal);
 const close=()=>modal.remove();modal.querySelector('.creative-modal-backdrop').onclick=close;modal.querySelector('[data-close-creative]').onclick=close;
 const canvas=modal.querySelector('#affiliateCreativeCanvas'),ctx=canvas.getContext('2d'),theme=material.theme||{};
 const grad=ctx.createLinearGradient(0,0,1080,1920);grad.addColorStop(0,theme.from||'#07152d');grad.addColorStop(1,theme.to||'#244eb9');ctx.fillStyle=grad;ctx.fillRect(0,0,1080,1920);
 ctx.fillStyle='rgba(255,255,255,.06)';ctx.beginPath();ctx.arc(920,180,370,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(90,1650,460,0,Math.PI*2);ctx.fill();
 const logo=new Image();logo.crossOrigin='anonymous';logo.src='./assets/oyag-approved-mark.png';await new Promise(resolve=>{logo.onload=resolve;logo.onerror=resolve});if(logo.complete&&logo.naturalWidth)ctx.drawImage(logo,90,100,170,170);
 ctx.fillStyle='#fff';ctx.font='700 42px system-ui';ctx.fillText('OYAG ECOSYSTEM',290,180);ctx.font='500 26px system-ui';ctx.fillStyle='rgba(255,255,255,.72)';ctx.fillText(material.product_name,292,226);
 ctx.fillStyle='#fff';ctx.font='800 76px system-ui';wrapCanvasText(ctx,material.headline,90,500,900,96);
 ctx.font='500 36px system-ui';ctx.fillStyle='rgba(255,255,255,.82)';wrapCanvasText(ctx,material.caption,90,850,900,52,5);
 const qrBox=document.createElement('div');qrBox.style.cssText='position:fixed;left:-9999px;top:-9999px;background:#fff;padding:18px';document.body.appendChild(qrBox);new QRCode(qrBox,{text:affiliateUrl,width:420,height:420,colorDark:'#07152d',colorLight:'#ffffff',correctLevel:QRCode.CorrectLevel.H});await new Promise(r=>setTimeout(r,80));const qr=qrBox.querySelector('canvas')||qrBox.querySelector('img');
 ctx.fillStyle='#fff';roundRect(ctx,300,1185,480,560,34);ctx.fill();if(qr)ctx.drawImage(qr,330,1215,420,420);qrBox.remove();
 ctx.fillStyle='#07152d';ctx.font='800 30px system-ui';ctx.textAlign='center';ctx.fillText(material.cta_text||'Acesse agora',540,1685);ctx.font='600 22px system-ui';ctx.fillStyle='#284363';ctx.fillText('Aponte a câmera para o QR Code',540,1725);ctx.textAlign='left';
 ctx.font='500 22px system-ui';ctx.fillStyle='rgba(255,255,255,.76)';wrapCanvasText(ctx,affiliateUrl,90,1830,900,30,2);
 modal.querySelector('#downloadCreative').onclick=()=>canvas.toBlob(blob=>{const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='oyag-'+material.product_code+'-status.png';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)},'image/png');
}
function wrapCanvasText(ctx,text,x,y,maxWidth,lineHeight,maxLines=6){const words=String(text||'').split(/\s+/);let line='',lines=0;for(let i=0;i<words.length;i++){const test=line+words[i]+' ';if(ctx.measureText(test).width>maxWidth&&line){ctx.fillText(line.trim(),x,y);line=words[i]+' ';y+=lineHeight;lines++;if(lines>=maxLines-1)break}else line=test}if(line&&lines<maxLines)ctx.fillText(line.trim(),x,y)}
