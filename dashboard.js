...
async function showAffiliateNetwork(){
 ...
 const materialCards=media.map(m=>{
  ...
  return '<article class="creative-card" data-material-card data-product="'+esc(m.product_code)+'" data-format="'+esc(m.format)+'"><div class="creative-preview format-'+esc(m.format)+'" style="--creative-from:'+esc(from)+';--creative-to:'+esc(to)+';--creative-accent:'+esc(accent)+'">'+(m.asset_url?'<img class="creative-product-image" src="'+esc(m.asset_url)+'" alt="'+esc(m.product_name)+'">':'')+'<div class="creative-preview-shade"></div><img class="creative-brand-mark" src="./assets/oyag-approved-mark.png" alt="OYAG"><div class="creative-preview-copy"><span>'+esc(m.product_name)+'</span><strong>'+esc(m.headline)+'</strong><small>'+esc(m.cta_text)+'</small></div></div><div class="creative-info"><div><span class="creative-format">'+esc(formatLabel[m.format]||m.format)+'</span><h3>'+esc(m.title)+'</h3></div><p>'+esc(m.caption)+'</p><div class="creative-actions"><button class="button secondary" data-copy-caption="'+esc(m.id)+'">Copiar legenda</button><button class="catalog-primary" data-personalize-material="'+esc(m.id)+'">Criar com meu link</button></div></div></article>';
 }).join('');
 ...
 C.querySelector('#affiliateCopy')?.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(affiliateUrl);alert('Link de afiliado copiado.')}catch{}});
...
async function generateAffiliateStatus(material,affiliateUrl){
 ...
 const formats={story:{width:1080,height:1920,label:'Story 1080×1920'},reel_cover:{width:1080,height:1920,label:'Capa de Reels 1080×1920'},whatsapp_status:{width:1080,height:1920,label:'Status 1080×1920'},feed_square:{width:1080,height:1080,label:'Feed 1080×1080'},feed_portrait:{width:1080,height:1350,label:'Feed vertical 1080×1350'}};
 const size=formats[material.format]||formats.whatsapp_status,w=size.width,h=size.height,scale=h/1920;
 const modal=document.createElement('div');modal.className='creative-modal';modal.innerHTML='<div class="creative-modal-backdrop"></div><section class="creative-modal-card"><div><p class="eyebrow">ARTE PERSONALIZADA</p><h2>'+esc(material.product_name)+'</h2><p>'+esc(size.label)+' com seu link e QR Code exclusivo.</p></div><canvas id="affiliateCreativeCanvas" width="'+w+'" height="'+h+'"></canvas><div class="creative-modal-actions"><button class="button secondary" data-close-creative>Fechar</button><button class="catalog-primary" id="downloadCreative">Baixar '+esc(size.label)+'</button></div></section>';document.body.appendChild(modal);
 ...
 const grad=ctx.createLinearGradient(0,0,w,h);grad.addColorStop(0,theme.from||'#07152d');grad.addColorStop(1,theme.to||'#244eb9');ctx.fillStyle=grad;ctx.fillRect(0,0,w,h);
 if(material.asset_url){const productImage=new Image();productImage.crossOrigin='anonymous';productImage.src=material.asset_url;await new Promise(resolve=>{productImage.onload=resolve;productImage.onerror=resolve});if(productImage.complete&&productImage.naturalWidth){const imageHeight=Math.round(h*.46),ratio=Math.max(w/productImage.naturalWidth,imageHeight/productImage.naturalHeight),dw=productImage.naturalWidth*ratio,dh=productImage.naturalHeight*ratio;ctx.globalAlpha=.72;ctx.drawImage(productImage,(w-dw)/2,0,dw,dh);ctx.globalAlpha=1}}
 const overlay=ctx.createLinearGradient(0,0,0,h);overlay.addColorStop(0,'rgba(3,12,31,.12)');overlay.addColorStop(.34,'rgba(3,12,31,.46)');overlay.addColorStop(.62,'rgba(3,12,31,.94)');overlay.addColorStop(1,'rgba(3,12,31,1)');ctx.fillStyle=overlay;ctx.fillRect(0,0,w,h);
 const logo=new Image();logo.crossOrigin='anonymous';logo.src='./assets/oyag-approved-mark.png';await new Promise(resolve=>{logo.onload=resolve;logo.onerror=resolve});const logoSize=Math.max(92,170*scale);if(logo.complete&&logo.naturalWidth)ctx.drawImage(logo,72,58,logoSize,logoSize);
 ctx.fillStyle='#fff';ctx.font='700 '+Math.max(30,42*scale)+'px system-ui';ctx.fillText('OYAG ECOSYSTEM',72+logoSize+28,105+logoSize*.25);ctx.font='500 '+Math.max(20,26*scale)+'px system-ui';ctx.fillStyle='rgba(255,255,255,.76)';wrapCanvasText(ctx,material.product_name,72+logoSize+28,150+logoSize*.25,w-logoSize-190,34*scale,2);
 const copyY=h*.46;ctx.fillStyle='#fff';ctx.font='800 '+Math.max(52,76*scale)+'px system-ui';wrapCanvasText(ctx,material.headline,72,copyY,w-144,Math.max(66,96*scale),3);
 ctx.font='500 '+Math.max(25,34*scale)+'px system-ui';ctx.fillStyle='rgba(255,255,255,.84)';wrapCanvasText(ctx,material.caption,72,copyY+Math.max(230,285*scale),w-144,Math.max(38,48*scale),h>1400?4:2);
 const qrSize=h>1400?300:220,cardW=qrSize+52,cardH=qrSize+128,cardX=w-cardW-72,cardY=h-cardH-92;
 const qrBox=document.createElement('div');qrBox.style.cssText='position:fixed;left:-9999px;top:-9999px;background:#fff;padding:18px';document.body.appendChild(qrBox);new QRCode(qrBox,{text:affiliateUrl,width:qrSize,height:qrSize,colorDark:'#07152d',colorLight:'#ffffff',correctLevel:QRCode.CorrectLevel.H});await new Promise(r=>setTimeout(r,80));const qr=qrBox.querySelector('canvas')||qrBox.querySelector('img');
 ctx.fillStyle='#fff';roundRect(ctx,cardX,cardY,cardW,cardH,30);ctx.fill();if(qr)ctx.drawImage(qr,cardX+26,cardY+26,qrSize,qrSize);qrBox.remove();
 ctx.fillStyle='#07152d';ctx.font='800 24px system-ui';ctx.textAlign='center';ctx.fillText(material.cta_text||'Acesse agora',cardX+cardW/2,cardY+qrSize+69);ctx.font='600 17px system-ui';ctx.fillStyle='#284363';ctx.fillText('Aponte a câmera',cardX+cardW/2,cardY+qrSize+101);ctx.textAlign='left';
 ctx.font='500 '+(h>1400?22:18)+'px system-ui';ctx.fillStyle='rgba(255,255,255,.78)';wrapCanvasText(ctx,affiliateUrl,72,h-72,w-cardW-180,30,3);
 modal.querySelector('#downloadCreative').onclick=()=>canvas.toBlob(blob=>{const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='oyag-'+material.product_code+'-'+material.format+'.png';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)},'image/png');
}
